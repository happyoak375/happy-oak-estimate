...
export default function Dashboard() {
    ...
    const [proposals, setProposals] = useState<(EstimateData & { id: string })[]>([]);
    ...
    useEffect(() => {
        async function loadData() {
            ...
            try {
                const data = await DatabaseService.getProposals() as (EstimateData & { id: string })[];

                // Sort by ID descending as a quick way to show newest first 
    ...
    const approvedTotal = proposals
        ...
        .reduce((total, prop) => {
            const propTotal = prop.jobAreas?.reduce((sum: number, area: JobArea) => sum + (Number(area.price) || 0), 0) || 0;
        ...
        <RequireAuth>
            <div className="min-h-screen bg-brand-canvas">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm border-t-4 border-t-brand-green">
                        ...
                        <p className="text-3xl font-bold text-brand-green">
                            ${approvedTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                    </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        <div className="p-8 min-h-[300px] flex items-center justify-center text-brand-blue font-semibold">
                            Loading pipeline data...
                        </div>
                    ) : proposals.length === 0 ? (
                        <div className="p-8 min-h-[300px] flex items-center justify-center text-gray-500">
                            No proposals found. Click &quot;Create New Estimate&quot; to get started!
                        </div>
                    ) : (
                        <div className="overflow-x-auto min-h-[300px]">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {proposals.map((prop) => {
                                        const total = prop.jobAreas?.reduce((sum: number, area: JobArea) => sum + (Number(area.price) || 0), 0) || 0;
                                        return (
                                            <tr key={prop.id} className="border-b border-gray-100 hover:bg-white transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`px-3 py-1 text-xs font-bold rounded-full uppercase tracking-wider ${getStatusBadge(prop.status as string)}`}>
                                                        {prop.status || "Draft"}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end items-center gap-3">
                                                        ...
                                                        <span className="text-gray-300">|</span>
                                                        <button
                                                            onClick={() => handleDelete(prop.id as string, prop.clientName || "this estimate")}
